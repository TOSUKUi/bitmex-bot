...
def execute(bins, conditions, client=None):
    ...
    rwvfs = [ta.reverse_williams_vix_fix(bins[i:], conditions.look_back_period_of_standard_deviation_high)
            for i in range(0, conditions.look_back_period_percentile_high + 1)]
    s_rwvfs = pd.Series(rwvfs)

    # 最低50足分のwvf, 最低2足分のrci
    if conditions.is_oversold(df_rci3lines[conditions.rci_overbought_trigger], s_wvfs):
        ...
        return True
    elif conditions.is_overbought(df_rci3lines[conditions.rci_oversold_trigger], s_rwvfs):
        if client is not None:
